...
namespace FoodFlowSystem.Services.Product
{
    public class ProductService : IProductService
    {
        ...
        public async Task<ProductResponse> AddAsync(CreateProductRequest request)
        {
            ...
            var checkProduct = _productRepository.IsExistProductAsync(request.Name);
        ...
        {
            var checkProduct = await _productRepository.IsExistProductAsync(id.ToString());
            if (checkProduct == null)
            {
        ...
        public async Task<ProductResponse> UpdateAsync(UpdateProductRequest request)
        {
            ...
            var product = await _productRepository.IsExistProductAsync(request.ID.ToString());
            if (product == null)
            {
                _logger.LogError("Product not found");
                throw new ApiException("Product not found", 404);
